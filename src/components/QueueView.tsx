...
/** Queue content, without its own SafeAreaView — used both as the pinned Queue tab (see
 * (tabs)/queue.tsx, which supplies the SafeAreaView) and, when unpinned via Settings > Customize
 * Tabs, rendered inline in More with an onBack button — same pattern as
 * HistoryView/StatsView/SettingsView/DownloadsView. */
export function QueueView({ onBack }: { onBack?: () => void }) {
    ...
    return (
      <ScaleDecorator>
        <ThemedView style={styles.row}>
          ...
          <Pressable onPress={() => removeEpisode(item.episodeId)} hitSlop={8} style={styles.deleteButton}>
  ...
  function renderPlayedItem(item: QueuedEpisode) {
    ...
    return (
      <ThemedView key={item.episodeId} style={[styles.row, styles.playedRow]}>
        ...
        <EpisodePlayButton playing={false} loading={false} progress={progress} onPress={() => handlePlayPause(item)} />
      </ThemedView>
    );
  }
  ...
  const nowPlayingArtwork = nowPlaying ? (nowPlaying.episode.artworkUrl ?? nowPlaying.podcastArtworkUrl) : null;
    ...
    <>
      ...
      {showSkeleton ? (
        ...
        <DraggableFlatList
          ...
          ListFooterComponent={<View style={{ height: BottomTabInset + Spacing.four + (nowPlaying ? MiniPlayerHeight : 0) }} />}
          ListHeaderComponent={
            <>
              {playedFromQueueVisible.length > 0 && (
                <ThemedView style={styles.playedSection}>
                  <Pressable onPress={() => setPlayedCollapsed((value) => !value)} style={styles.playedHeader}>
                    <ThemedText type="smallBold" themeColor="textSecondary">
